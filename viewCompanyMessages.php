...
$chat->markMessagesAsSeen($emp_id, $user_id);
$empData = $base->selectCompanyForProfilesWithID($emp_id);
...
<body>
    <main>
        <div class="glass-bg py-3 px-4 rounded-2 col-md-7 position-relative">
            <a href="./userInbox.php" class="text-decoration-none optional-color d-flex align-items-center gap-2"><i
                    class="bi bi-arrow-left-circle-fill fs-4"></i> Go
                Back</a>
            <div class="d-flex align-items-center flex-column gap-2">
                <div class="secondary-bg py-2 px-3 rounded-pill fs-5 fw-bold optional-color">
                    <?php echo $_SESSION['logged']['username']; ?>
                </div>
                <small><i class="bi bi-arrow-down-circle-fill text-white"></i></small>
                <h3 class="fw-bold text-white">
                    <?php echo $empData['company_name']; ?>
                </h3>
            </div>
            <hr>
            <input type="hidden" id="emp_id" data-recruiter-id="<?php echo $emp_id; ?>">
            ...
            <?php
            if ($chat->isAllowToMessage($user_id, $emp_id)) {
                ?>
                <div
    ...
    ?>
    <script src="./Scripts/inboxFetchMessage.js?v=<?php echo time(); ?>"></script>
</body>
