...
if ($_SERVER['REQUEST_METHOD'] == 'POST') {
    ...
    if (isset($_FILES['profilePicture'])) {
        ...
        if ($user_info = $baseClass->selectCompanyWithProfiles($user_id)) {
            $targetFilePath = $filePath . basename($_FILES['profilePicture']['name']);
            $escaped_string_picture = $conn->real_escape_string($targetFilePath);

            if (file_exists($user_info['profile_picture'])) {
                if (unlink($user_info['profile_picture']) && move_uploaded_file($_FILES['profilePicture']['tmp_name'], $targetFilePath) && $advanceClass->updateCompanyLogo($user_id, $escaped_string_picture)) {
                    $_SESSION['generalSuccess'] = "Profile Picture Updated Successfully";
                } else {
                    $_SESSION['generalError'] = "Failed to Update Profile Picture!";
                }
            } else {
                if (move_uploaded_file($_FILES['profilePicture']['tmp_name'], $targetFilePath) && $advanceClass->updateCompanyLogo($user_id, $escaped_string_picture)) {
                    $_SESSION['generalSuccess'] = "Profile Picture Updated Successfully";
                } else {
                    $_SESSION['generalError'] = "Failed to Update Profile Picture!";
                }
